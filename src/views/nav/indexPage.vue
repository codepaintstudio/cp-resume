    ...
    <div class="absolute right-0 mr-[3vw] top-1/2 transform -translate-y-1/2">
      ...
      <div v-if="userStore.isLoggedIn" class="relative">
        <div class="text-[0.8vw] text-[#4F4F4F] no-underline cursor-pointer flex justify-between items-center">
          ...
          <ChevronDown @mouseenter="handleMouseEnter" @mouseleave="handleMouseLeave" />
        </div>
        ...
        <div v-if="showDropdown" class="absolute top-full right-0 mt-2 w-[8vw] bg-white shadow-lg rounded-md py-2 z-50"
          ...
          <div class="px-4 py-2 hover:bg-[rgb(231,238,254)] hover:text-[#3370FF] cursor-pointer text-[0.8vw] "
            @click="myResume">
            我的简历
          </div>
