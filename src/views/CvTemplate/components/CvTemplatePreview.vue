    ...
    <div
      ...
      <div class="flex mt-[1rem] justify-center md:flex-row items-center md:items-start gap-6">
        ...
        <div class="space-y-5 mt-[50px]">
          <h1 class="text-2xl font-bold">{{ cvData.title }}</h1>
          <div class="flex gap-2">
            <span class="px-2 py-1 text-sm bg-gray-200 rounded">{{ cvData.style }}</span>
            <span class="px-2 py-1 text-sm bg-gray-200 rounded">{{ cvData.industry }}</span>
            <span class="px-2 py-1 text-sm bg-gray-200 rounded">{{ cvData.color }}</span>
          </div>

          <p class="text-gray-600 text-sm color-[#595b5e]">尺寸：{{ cvData.size }}</p>
          <p class="text-gray-600 text-sm color-[#595b5e]">颜色：{{ cvData.color }}</p>
